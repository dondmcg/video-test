...
var mediaFrag = ( function() {
    ...
    document.getElementById('add_frag').addEventListener('click', function(e) {
        ...
        newRow = '<div class="row fragment">\n<div class="col-sm-9">' +
            ...
            '</small></div></div></div></form></div><div class="col-sm-1 bg-lt-grey">' +
            '<span data-start="' + start + '" data-end="' + end + '" class="glyphicon glyphicon-play"></span></div>' +
            '<div class="col-sm-1 bg-primary"><span class="glyphicon glyphicon-pencil"></span></div>' +
            '<div class="col-sm-1 bg-delete"><span class="delete glyphicon glyphicon-remove" data-toggle="modal" data-target="#confirm-delete" data-row="' + fragments.length + '" data-frag-title="' + title + ' ' + start + ' - ' + end + '"></span>' +
            '</div>\n</div>';
        if( title && start && end ){
...
window.onload = function() {
    ...
    // populate loading image in video player
